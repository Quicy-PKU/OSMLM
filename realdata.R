...
save(res, file=paste('realdata/beta/', 'mv', '.RData',sep=''))
mse = calc_mse(res, beta_full)
for (i in 1:length(mse)) {
    MSE[2, i] = mse[i]
}
print(mse)
write.csv(MSE, 'realdata/po_mv.csv')
...
save(res, file=paste('realdata/beta/', 'mvc', '.RData',sep=''))
mse = calc_mse(res, beta_full)
for (i in 1:length(mse)) {
    MSE[3, i] = mse[i]
}
print(mse)
write.csv(MSE, 'realdata/po_mvc.csv')
...
save(res, file=paste('realdata/beta/', 'fmv_rp', '.RData',sep=''))
mse = calc_mse(res, beta_full)
for (i in 1:length(mse)) {
    MSE[4, i] = mse[i]
}
print(mse)
write.csv(MSE, 'realdata/fmv_rp.csv')
...
save(res, file=paste('realdata/beta/', 'fmv_rs', '.RData',sep=''))
mse = calc_mse(res, beta_full)
for (i in 1:length(mse)) {
    MSE[5, i] = mse[i]
}
print(mse)
write.csv(MSE, 'realdata/fmv_rs.csv')

end = Sys.time()
print(end - start)
write.csv(MSE, 'realdata/po.csv')